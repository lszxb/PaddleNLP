...
static std::unordered_map<CUdeviceptr, VirtualAllocationMetadata>
    virtual_allocation_metadata;
...
static void dealloc_virtual_allocation(phi::Allocation* ptr) {
  ...
  // cuMemUnmap does not synchronize with the in-flight kernels, so we need to
  // sync here
  int current_device = -1;
  PD_CHECK(cudaGetDevice(&current_device) == CUDA_SUCCESS,
           "cudaGetDevice before unmap failed");
  ...
  PD_CHECK(cudaSetDevice(current_device) == CUDA_SUCCESS,
           "cudaSetDevice after unmap failed");

  virtual_allocation_metadata.erase((CUdeviceptr)ptr->ptr());
}


static phi::DenseTensor* alloc_vtensor(const phi::DenseTensorMeta& meta,
                                       const phi::Place& place) {
  PD_CHECK(place.GetType() == phi::AllocationType::GPU,
...
static phi::DenseTensor* expand_one_token(const phi::DenseTensor& self) {
  ...
  if (required_size <= new_tensor->capacity()) {
    ...
    auto& metadata =
        virtual_allocation_metadata.at((CUdeviceptr)new_tensor->data());

    PD_CHECK(required_size <= metadata.reserved_size,
             "reserved size is not enough");

    size_t real_alloc_size = required_size - new_tensor->capacity();
    CUdeviceptr real_alloc_ptr =
        (CUdeviceptr)new_tensor->data() + new_tensor->capacity();
...
std::vector<paddle::Tensor> VTensorReserveOneToken(
  ...
  if (virtual_allocation_metadata.find((CUdeviceptr)cache.data()) !=
      virtual_allocation_metadata.end()) {
    PD_CHECK(get_max_stride(cache.strides()) == cache.strides()[1]);
    vtensor = std::static_pointer_cast<phi::DenseTensor>(cache.impl());
  } else {
    phi::DenseTensorMeta meta(
        cache.dtype(), cache.dims(), compute_transposed_strides(cache));
    paddle::Tensor new_tensor{
        std::shared_ptr<phi::TensorBase>(alloc_vtensor(meta, cache.place()))};
    paddle::experimental::assign_out_(cache, new_tensor);
    vtensor = std::static_pointer_cast<phi::DenseTensor>(new_tensor.impl());
  }

  paddle::Tensor new_tensor{
      std::shared_ptr<phi::TensorBase>(expand_one_token(*vtensor))};

  // This is slow, just use direct copy instead
  // paddle::experimental::set_value_with_tensor_(new_tensor, append_state,
  // {-1}, {INT32_MAX}, {1}, {1}, {}, {});
  size_t dtype_size = phi::SizeOf(new_tensor.dtype());
  size_t offset =
      (new_tensor.shape()[1] - 1) * new_tensor.strides()[1] * dtype_size;
  cudaMemcpyAsync((void*)((CUdeviceptr)new_tensor.data() + offset),
                  append_state.data(),
                  append_state.numel() * dtype_size,
                  cudaMemcpyDeviceToDevice,
                  append_state.stream()  // FIXME: Tensor.stream() is deprecated
  );
  return {new_tensor};
}
...
std::vector<paddle::DataType> VTensorReserveOneTokenInferDtype(
    const paddle::DataType& cache_dtype, const paddle::DataType& append_state) {
  return {cache_dtype};
}
